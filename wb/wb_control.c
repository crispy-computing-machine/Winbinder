					   ...
					   int xPos, int yPos, int nWidth, int nHeight, UINT id, DWORD dwWBStyle, long lParam, int nTab)
{
	...
	if (pwboParent->uClass == TabControl)
	{
		...
		hTab = pTabData->page[nTab].hwnd ? pTabData->page[nTab].hwnd : pwboParent->hwnd;
	}
	...
	case Frame:
		if ((nWidth > nHeight) && nHeight <= 8)
		{ // Horizontal line
			pszClass = TEXT("STATIC");
			dwStyle = WS_CHILD | SS_ETCHEDHORZ | nVisible;
		}
		else if ((nWidth < nHeight) && nWidth <= 8)
		{ // Vertical line
			pszClass = TEXT("STATIC");
			dwStyle = WS_CHILD | SS_ETCHEDVERT | nVisible;
		}
		else if (BITTEST(dwWBStyle, WBC_IMAGE))
		{ // Image
			pszClass = TEXT("STATIC");
...
PWBOBJ wbGetControl(PWBOBJ pwboParent, int id)
{
	...
	if (!wbIsWBObj(pwboParent, TRUE))
	{ // Is it a valid control?
	...
	if (pwboParent->uClass == Menu)
	{
		...
		if (IsMenu((HMENU)pwboParent->hwnd))
		{ // Is it a menu?
	...
	else if (!IsWindow(pwboParent->hwnd))
	{ // Is it a window?
	...
	{ // Yes, it's a window
		...
		default: // Regular window classes
		{
			...
			if (hCtrl)
			{
				...
				if (IsWindow(hCtrl))
				{
					pwbo = (PWBOBJ)GetWindowLongPtr(hCtrl, GWLP_USERDATA);
				}
				else
				{
					pwbo = wbMalloc(sizeof(WBOBJ));
...
BOOL wbSetEnabled(PWBOBJ pwbo, BOOL bState)
{
	...
	if (pwbo->uClass == Menu && IsMenu((HMENU)pwbo->hwnd))
	{ // Is it a menu item?
