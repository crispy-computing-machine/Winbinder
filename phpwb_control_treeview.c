...
ZEND_FUNCTION(wbtemp_create_treeview_item)
{
	...
	default:
		ret = (LONGLONG)wbAddTreeViewItemLevel((PWBOBJ)pwbo, where, wcs, lparam, setlparam, img1, img2);
		break;
	case 1: // 'where' is the sibling of the new node
		ret = (LONGLONG)wbAddTreeViewItemSibling((PWBOBJ)pwbo, (HTREEITEM)where, wcs, lparam, setlparam, img1, img2);
		break;
	case 2: // 'where' is the parent of the new node
		ret = (LONGLONG)wbAddTreeViewItemChild((PWBOBJ)pwbo, (HTREEITEM)where, wcs, lparam, setlparam, img1, img2);
		break;
	}
	RETURN_LONG(ret);
}
